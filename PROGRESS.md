...
- [x] **数学記号読み上げ**（＝→イコール、＋→プラス、×→かける、÷→わる、−→マイナス）
- [x] **改行での休止**（SSML breakタグで300ms/500ms休止）
- [x] **UI簡素化**（エンジン固定、女声/男声ボタン、速度ボタン）
...
### SSML休止
- **読点より確実**: Google Cloud TTSでは読点での休止が不安定、SSMLの`<break>`タグが確実
- **使い分け**: 単一改行→300ms、空行→500ms
- **XMLエスケープ必須**: `&<>\"'`

### 記号読み上げ
- **イコールのみ特別扱い**: 前後に読点追加（`、イコール、`）で一拍の休止
- **算術記号**: 休止なし（`プラス`、`マイナス`等）
- **長音記号との区別**: `ー`(U+30FC)と`−`(U+2212)はUnicodeで区別可能
